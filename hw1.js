...
const Product = class Product {
    ...
    static halfOff = (products) => { 
        const arr = new Array();
        products.forEach((products) => arr.push(new Product(`${products.name}, ${products.price*0.5}, ${products.availability}`)));
        return arr;
    }
    ...
    static printProducts = (products) => {products.forEach(product => {
        let inStock = "No";
        if (product.availability === "In Stock") {inStock = "Yes"};
        console.log(`Product: ${product.name}, Cost: $${product.price.toFixed(2)}, Availability: ${inStock}`)
